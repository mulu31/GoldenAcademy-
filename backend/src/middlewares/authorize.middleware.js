...
export const authorizeDepartmentAdmin = catchAsync(async (req, _res, next) => {
  ...
  if (!req.user.departmentId) {
    throw new ApiError(403, 'Department admin must be associated with a department');
  }
 ...
 */
export const authorizeTeacherForSubject = catchAsync(async (req, _res, next) => {
  ...
  if (result.rows.length === 0) {
    throw new ApiError(403, 'Not authorized to teach this subject in this class');
  }
...
async function getResourceDepartmentId(req) {
  ...
  const teacherIdToCheck = teacherId || (req.path.includes('/teachers') ? id : null);
  if (teacherIdToCheck) {
    const result = await pool.query(
